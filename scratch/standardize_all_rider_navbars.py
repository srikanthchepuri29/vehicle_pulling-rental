...
def get_clean_rider_navbar(active_key):
...
for fname, key in rider_pages.items():
    ...
    if os.path.exists(fpath):
        ...
        if nav_start != -1:
            ...
            if nav_end != -1:
                new_nav = get_clean_rider_navbar(key)
                content = content[:nav_start] + new_nav + content[nav_end + 6:]

        with open(fpath, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"Standardized navbar applied to {fname}")

print("All 5 rider navbars standardized with clean flex alignment!")
