...
def get_compact_navbar_html(active_key):
    ...
    <nav class="navbar navbar-expand-lg premium-navbar p-0">
        <div class="container-fluid d-flex align-items-center justify-content-between">
            <div class="d-flex align-items-center">
                <a class="nav-menu-trigger dropdown-toggle mr-2" href="#" id="dashboardDropdown" data-toggle="dropdown" style="color: #fff; text-decoration: none;">
...
new_css = """.premium-navbar .nav-link { color:#fff !important; font-weight:500 !important; font-size:13.5px !important; padding:0 11px !important; height:60px !important; display:flex !important; align-items:center; text-decoration:none !important; transition:.25s; }"""
...
for fname, active_key in rider_pages.items():
    ...
    if os.path.exists(fpath):
        ...
        # Update CSS nav-link padding
        content = re.sub(r'\.premium-navbar \.nav-link \{[^}]+\}', new_css, content)

        nav_start = content.find('<nav class="navbar')
        if nav_start != -1:
            nav_end = content.find('</nav>', nav_start)
            if nav_end != -1:
                new_nav = get_compact_navbar_html(active_key)
                content = content[:nav_start] + new_nav + content[nav_end + 6:]

        with open(fpath, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"Compact navbar updated in {fname}")

print("Compact, seamless navbar layout without middle space gaps applied successfully!")
